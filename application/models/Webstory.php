...
class Webstory extends MY_Model {
	...
	protected function getWhere($params){
		...
		$where = "WHERE 1=1 ";
		if(!empty($search)){
			$where .= "AND (titulo_simple like '%".$this->db->escape_like_str($search)."%' OR identificador like '%".$this->db->escape_like_str($search)."%' OR url like '%".$this->db->escape_like_str($search)."%' ) ";
		}
	...
	public function buscarSelectPropuesta($idpropuesta, $search){
		$where = empty($search)? '' : "titulo like '%".$this->db->escape_like_str($search)."%' AND";
