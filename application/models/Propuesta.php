...
class Propuesta extends MY_Model {
	...
	protected function getWhere($params){
		...
		$where = "WHERE 1=1 ";
		if(!empty($search)){
			$where .= "AND (titulo_simple like '%".$this->db->escape_like_str($search)."%' OR identificador like '%".$this->db->escape_like_str($search)."%' OR anio like '%".$this->db->escape_like_str($search)."%' OR sector_productivo like '%".$this->db->escape_like_str($search)."%' OR elestado like '%".$this->db->escape_like_str($search)."%') ";
		}
	...
	public function buscarSelect($search){
		...
		$query=$this->db->query("SELECT idpropuesta AS id, CONCAT(titulo_simple, ' (', identificador, ')') as 'text' 
									FROM v_Propuesta 
									WHERE (titulo_simple like ? OR identificador like ?) $where",  Array('%'.$search.'%', '%'.$search.'%'));
		$resultado=$query->result_array();		
		return $resultado;
	}
	...
	public function getGraphData($codlang, $campo, $tipo, $filtros){
		...
		foreach($tipos as $k=>$t){
			if(!empty($filtros[$k])){
				...
				$where .= ' AND '.$aux;
				//print_r($filtros['tipo_investigacion']); 
			}
		}
		//echo $where; exit;
		$rango = $filtros['rango'];
	...
	public function buscar($data){
		...
		if(!empty($data['keyword'])){
			...
			foreach($palabras as $pal){
				...
				$where.=" AND (titulo_completo like '%".$this->db->escape_like_str($palabra)."%' OR titulo_simple like '%".$this->db->escape_like_str($palabra)."%' OR anio=".$this->db->escape($palabra)." OR identificador like '%".$this->db->escape_like_str($palabra)."%')";
			}			
		}
