...
class Organismo extends MY_Model {
	...
	protected function getWhere($params){
		...
		$where = "WHERE 1=1 ";
		if(!empty($search)){
			$where .= "AND (nombre like '%".$this->db->escape_like_str($search)."%' OR nombre_largo like '%".$this->db->escape_like_str($search)."%' OR tipo like '%".$this->db->escape_like_str($search)."%' OR enuso like '%".$this->db->escape_like_str($search)."%') ";
		}
		return $where;
	}
	...
	public function buscarSelect($search){
		$query=$this->db->query("SELECT o.idorganismo AS id, CONCAT(o.nombre, ' - ',COALESCE(pal.nombre,''), ' - ',COALESCE(o.nombre_largo,'')) as 'text', o.idpais, o.tipo_institucion 
									...
									WHERE o.nombre like ? AND o.habilitado=1", Array('%'.$search.'%'));
		$resultado=$query->result_array();		
		return $resultado;
	}
}
