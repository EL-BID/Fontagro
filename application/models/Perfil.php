...
class Perfil extends MY_Model {
	...
	protected function getWhere($params){
		...
		if(!empty($search)){
			$where .= "AND (idperfil like '%".$this->db->escape_like_str($search)."%' OR titulo_corto like '%".$this->db->escape_like_str($search)."%' OR titulo like '%".$this->db->escape_like_str($search)."%' OR nombre like '%".$this->db->escape_like_str($search)."%' OR email like '%".$this->db->escape_like_str($search)."%' OR estado like '%".$this->db->escape_like_str($search)."%' ) ";
		}
		return $where;
	}
	...
	public function buscarSelect($search){
		$query=$this->db->query("SELECT idperfil AS id, CONCAT('ID:', idperfil, ' - ', titulo_corto) as 'text' 
									FROM Perfil 
									WHERE (titulo_corto like ? OR idperfil = ? ) AND idestadoperfil=3", Array('%'.$search.'%', $search));
		$resultado=$query->result_array();		
		return $resultado;
	}
