...
class Usuario extends MY_Model {
	...
	protected function getWhere($params){
		...
		$where = "WHERE 1=1 ";
		if(!empty($search)){
			$where .= "AND (email like '%".$this->db->escape_like_str($search)."%' OR tipo like '%".$this->db->escape_like_str($search)."%' OR nombre like '%".$this->db->escape_like_str($search)."%' OR institucion like '%".$this->db->escape_like_str($search)."%' OR propuestas like '%".$this->db->escape_like_str($search)."%') ";
		}
		return $where;
	}
