...
class Noticia extends MY_Model {
	...
	protected function getWhere($params){
		...
		if(!empty($search)){
			$where .= "AND (titulo like '%".$this->db->escape_like_str($search)."%' OR tiponoticia like '%".$this->db->escape_like_str($search)."%' OR aprobada like '%".$this->db->escape_like_str($search)."%') ";
		}
		return $where;
	}
